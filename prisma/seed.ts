...
async function main() {
  ...
  if (!user) {
    throw new Error(
      "No hay ningún usuario en la base de datos. Crea uno desde la app (Sign up en http://localhost:3000/sign-up) y vuelve a ejecutar: bun run db:seed"
    );
  }

  const userId = user.id;
  console.log(`Using user: ${user.name} (${user.email}) for seed properties.`);
